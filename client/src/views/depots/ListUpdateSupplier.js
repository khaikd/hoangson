...
import ItemUpdateSupplier from "./ItemUpdateSupplier";
import NumberFormat from 'react-number-format';

const ListUpdateSupplier = ({ totalDepot, arrSuppliers, setArrSuppliers, setTotal, numberKey }) => {

    const [allTotal, setAllTotal] = useState(totalDepot)
        ...
        <Fragment>
            <div className="p-0">
                ...
                <tbody>
                    {
                        Array(numberKey).fill(1).map((option, index) => (
                            <ItemUpdateSupplier key={index} itemSupplier={arrSuppliers[index]} arrSuppliers={arrSuppliers} setArrSuppliers={setArrSuppliers} keyItem={index} itemTotal={itemTotal} />
                            )
                        )
                    }
                    <tr>
...
export default ListUpdateSupplier
