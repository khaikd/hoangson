...
const AddStaff = () => {
    ...
    // Context
    const { showAddStaff, setAddStaff, addStaff, setShowToast } = useContext(StaffContext)
    ...
    const onChangeValue = event => setNewStaff( {...newStaff, [event.target.name]: event.target.value } )

    const onSubmit = async event => {
        event.preventDefault()
        const {success, message} = await addStaff(newStaff)
        closeDialog()
    ...
    const handleChangeSelect = (newValue, actionMeta) => {
        ...
        if(data){
            ...
            if(actionMeta.name === "level"){
                setNewStaff( {...newStaff, level: dataValue } )
            }
            if(actionMeta.name === "status"){
                setNewStaff( {...newStaff, status: dataValue } )
            }
        }
    }
    ...
    return (
        <Modal show={showAddStaff} onHide={closeDialog}
            size="lg"
